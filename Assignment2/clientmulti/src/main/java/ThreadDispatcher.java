...
public class ThreadDispatcher implements Runnable {
  ...
  private void phaseStart(ExecutorService pool, int phaseThread, int skierRange, int startTime,
      int endTime, int numPost, List<Future<List<RequestResult>>> response, boolean addition)
    ...
    for (int i = 0; i < phaseThread; i++) {
      Callable<List<RequestResult>> newThread = new SkiApiHandlerCallable(phaseLatch, successCount,
          ...
          endTime, params.getSkiLifts(), numPost, addition);
  ...
  public void run() {
    ...
    // Start phase one
    try {
      phaseStart(pool, phaseOneThreads, skierRange, startTime, endTime, numPost, response, false);
    ...
    // Start phase two.
    try {
      phaseStart(pool, phaseTwoThreads, skierRange, startTime, endTime, numPost, response, false);
    } catch (InterruptedException e) {
      e.printStackTrace();
    }
    ...
    // Start phase three.
    try {
      phaseStart(pool, phaseThreeThreads, skierRange, startTime, endTime, numPost, response, true);
    } catch (InterruptedException e) {
      e.printStackTrace();
    }
